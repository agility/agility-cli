...
describe('Authentication Validation Tests', () => {
  ...
  describe('No Authentication Available', () => {
    it('should initiate Auth0 flow when no PAT or cached tokens are available', async () => {
      ...
      const result = await runCLICommand('pull', [
        '--sourceGuid', 'test-guid-u', // Use a valid format GUID but non-existent
        '--locale', 'en-us',
        '--channel', 'website',
        '--headless',
        '--elements', 'Models'
      ...
      const hasAuth0Flow = /starting auth flow|waiting for authentication|browser|no token found in keychain/i.test(output);
    ...
    it('should fail with invalid PAT format', async () => {
      const result = await runCLICommand('pull', [
        ...
        '--locale', 'en-us', 
        '--channel', 'website',
        '--token', 'invalid-token-123', // Invalid format
        '--headless',
        '--elements', 'Models'
      ], {
        timeout: 30000,
    ...
    it('should handle empty PAT by falling back to Auth0 flow', async () => {
      ...
      const hasExpectedBehavior = /no token found|authentication|starting auth flow|timeout|waiting for authentication/i.test(output);
  ...
  describe('Environment Variable Clearing', () => {
    it('should not use environment variables when explicitly cleared', async () => {
      ...
      // If authentication succeeded, it means cached tokens were used
      if (result.exitCode === 0) {
        console.log('ℹ️  Command succeeded despite cleared environment variables - likely using cached authentication');
        console.log('💡 To test failure scenarios, run: npm run clear-tokens');
        return;
      }
      ...
      const hasAuthRelatedMessage = /no token|authentication|login|keychain|starting auth flow|timeout|waiting for authentication/i.test(output);
