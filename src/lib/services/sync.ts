...
// Removed ComprehensiveAnalysisRunner - using built-in analysis methods
// Removed ChainBuilder - not used in current implementation
// Removed TopologicalTwoPassOrchestrator - not used in current implementation
...
import { ChainDataLoader, MappingDependencyEnforcer } from '../utilities';
...
export class Sync {
    ...
    constructor(dryRun: boolean = false) {
        ...
        this.options = state.mgmtApiOptions;
        this.multibar = null; // No longer used
        this.sourceGuid = state.sourceGuid;
        this.targetGuid = state.targetGuid;
        this.locale = state.locale;
        this.channel = state.channel;
        this.isPreview = state.preview;
        this.blessedUIEnabled = state.blessed;
        this.isHeadless = state.headless;
        this.isVerbose = state.verbose;
        this.elements = state.elements.split(",");
        this.rootPath = state.rootPath;
        this.legacyFolders = state.legacyFolders;
        this.dryRun = dryRun;
        this.syncOptions = {
            debug: state.debug || state.test,
            maxDepth: state.maxDepth,
            forceUpdate: state.forceUpdate
        };
        this.fileOps = new fileOperations(this.rootPath, this.sourceGuid, this.locale, this.isPreview);
    ...
    async syncInstance(): Promise<void> {
        ...
        try {
            ...
            if (this.hasNoContent(sourceData)) {
                ...
                console.log(ansiColors.gray(`🔄 Triggering: pull --sourceGuid ${this.sourceGuid} --locale ${this.locale} --channel ${this.channel}`));
                ...
                // Create pull operation using state
                ...
                try {
                    ...
                    console.log(ansiColors.gray(`   node dist/index.js pull --sourceGuid ${this.sourceGuid} --locale ${this.locale} --channel ${this.channel} --verbose`));
                    return;
                }
            }
    ...
    private async enrichPagesWithSitemapParents(pages: any[]): Promise<any[]> {
        try {
            ...
            const hierarchy = sitemapHierarchy.buildPageHierarchy(nestedSitemap) as { [parentId: string]: number[] };
