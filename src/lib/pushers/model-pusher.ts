...
function updateFields(existingModel: mgmtApi.Model, sourceModel: mgmtApi.Model): mgmtApi.ModelField[] {
  ...
  sourceModel.fields.forEach((sourceField) => {
    const existingFieldIndex = existingModel.fields.findIndex(
      (existingField) => existingField.name === sourceField.name
    );
...
function cleanFieldSettings(fields: any[]): any[] {
  return fields.map((field) => {
    if (field.settings) {
      ...
      field.settings = Object.fromEntries(Object.entries(field.settings).filter(([, v]) => v !== ""));
...
export async function pushModels(
  ...
  const { targetGuid, modelDiffs, forceUpdate } = state;
  ...
  const processModel = async (
    ...
    try {
      ...
      if (existingModel) {
        if (isStubCreationIntent) {
          ...
          console.log(
            `✓ ${passDescription} ${ansiColors.underline(originalModelReferenceName)} ${ansiColors.bold.gray(
              "already exists"
            )} - Skipping stub creation. ${ansiColors.green(targetGuid)}: ${ansiColors.green(String(existingModel.id))}`
          );
        ...
        if (forceUpdate || areModelsDifferent(fixedModel, fixedExistingModel, modelDiffs, referenceMapper)) {
          try {
            ...
            // console.log(
            //   `📤 [PAYLOAD FIELDS] ${originalModelReferenceName}:`,
            //   modelPayload?.fields ? modelPayload.fields.map((f) => `${f.name}(${f.type})`) : "NO FIELDS"
            // );
            // console.log(
            //   `📥 [RESPONSE FIELDS] ${originalModelReferenceName}:`,
              ...
              modelPayload.fields.forEach((payloadField) => {
                ...
                if (responseField) {
                  ...
                  if (payloadField.label !== responseField.label) {
                    console.log(
                      `  📝 ${payloadField.name}: label changed from '${payloadField.label}' to '${responseField.label}'`
                    );
                  }
                }
              });
            ...
            try {
              ...
              const freshModel = await apiClient.modelMethods.getModelByReferenceName(
                originalModelReferenceName,
                targetGuid
              );
              if (freshModel) {
                // LOG POST-UPDATE API RESPONSE for debugging
                // console.log(
                //   `📡 [POST-UPDATE API RESPONSE] ${originalModelReferenceName}:`,
                //   JSON.stringify(freshModel, null, 2)
                // );
                // console.log(
                //   `📡 [POST-UPDATE FIELDS] ${originalModelReferenceName}:`,
                ...
                //   console.warn(
                //     `⚠️  Field count mismatch after update: expected ${payloadFieldCount}, got ${freshFieldCount}`
                //   );
                ...
                //   console.warn(
                //     `⚠️  API response vs fresh fetch field count differs: response=${responseFieldCount}, fresh=${freshFieldCount}`
                //   );
                ...
                //   console.log(
                //     `🔍 [CACHE VERIFICATION] ${originalModelReferenceName}: cache now has ${verifyFieldCount} fields`
                //   );
                //   if (verifyTarget.id !== freshModel.id) {
                //     console.warn(
                //       `⚠️  [CACHE VERIFICATION] Cache target ID (${verifyTarget.id}) differs from fresh model ID (${freshModel.id})`
                //     );
                //   }
                //   if (verifyFieldCount !== freshFieldCount) {
                //     console.warn(
                //       `⚠️  [CACHE VERIFICATION] Cache field count (${verifyFieldCount}) differs from fresh model field count (${freshFieldCount})`
                //     );
                //   }
                // } else {
                //   console.warn(
                //     `⚠️  [CACHE VERIFICATION] Could not retrieve updated mapping for ${originalModelReferenceName}`
                //   );
                // }
              } else {
            ...
            console.log(
              `✓ ${passDescription} ${ansiColors.bold.cyan("updated")} ${ansiColors.underline(
                originalModelReferenceName
              )} (ID: ${existingModel.id}) on target.`
            );
            processedModels.add(model.id); // Add to processed set using model ID
            return true;
          } catch (error: any) {
            ...
            if (isNoChangeNeededError) {
              ...
              console.log(
                `✓ ${passDescription} ${ansiColors.underline(originalModelReferenceName)} ${ansiColors.bold.gray(
                  "already up-to-date"
                )} (no update needed). ${ansiColors.green(targetGuid)}: ${ansiColors.green(String(existingModel.id))}`
              );
            ...
            console.error(
              ansiColors.red(
                `Error updating model ${originalModelReferenceName} (Target ID: ${
                  existingModel?.id || "N/A"
                }) - (${passDescription}):`
              ),
              error
            );
            ...
            if (error.request) {
              if (typeof error.request === "string") {
                ...
                console.error(
                  ansiColors.red(
                    `API Request Details: ${JSON.stringify(
                      {
          ...
          console.log(
            ...
            )} - Skipping update. ${ansiColors.green(targetGuid)}: ${ansiColors.green(String(existingModel.id))}`
          );
          processedModels.add(model.id); // Add to processed set using model ID
          return true;
        }
      ...
      if (isNotFoundError) {
        ...
        console.error(
          `[Model] ✗ Error during initial check for ${passDescription}, model ${originalModelReferenceName}: ${error.message}`
        );
        failed++;
        return false;
      }
    ...
    try {
      ...
      console.log(
        `✓ ${passDescription} ${ansiColors.underline(originalModelReferenceName)} ${ansiColors.bold.cyan(
          "created"
        )} (New ID: ${savedModel.id}) on target.`
      );
      processedModels.add(model.id); // Add to processed set using model ID
      return true;
    } catch (error: any) {
      ...
      if (isAlreadyExistsError) {
        ...
        console.log(
          `⚠️  ${passDescription} ${ansiColors.underline(
            originalModelReferenceName
          )} appears to already exist on target (creation failed). Attempting to retrieve...`
        );
        try {
          const retrievedModel = await apiClient.modelMethods.getModelByReferenceName(
            originalModelReferenceName,
            targetGuid
          );
          if (retrievedModel) {
            ...
            console.log(
              `✓ ${passDescription} ${ansiColors.underline(originalModelReferenceName)} ${ansiColors.bold.gray(
                "found on target"
              )} after creation failure. ${ansiColors.green(targetGuid)}: ${ansiColors.green(
                String(retrievedModel.id)
              )}`
            );
            processedModels.add(model.id);
            return true;
          ...
          console.error(
            `[Model] ✗ Failed to retrieve model ${originalModelReferenceName} after creation failure: ${retrieveError.message} (${passDescription})`
          );
        }
      }

      console.error(ansiColors.red(`Error creating model ${originalModelReferenceName} (${passDescription}):`), error);
      if (error.response && error.response.data) {
        console.error(ansiColors.red(`  API Response Data: ${JSON.stringify(error.response.data, null, 2)}`));
      }
      if (error.request) {
  ...
  console.log(
    ansiColors.yellow(
      `${successful} unique models processed successfully out of ${totalModels} total models (${failed} failed attempts)`
    )
  );
