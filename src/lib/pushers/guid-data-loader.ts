...
export class GuidDataLoader {
  ...
  async loadGuidEntities(
    ...
    if (elements.includes('Containers')) {
      const { getListsFromFileSystem, getContainersFromFileSystem } = await import(
        '../getters/filesystem/get-containers'
      );
    ...
    if (elements.includes('Content')) {
      const { getContentItemsFromFileSystem } = await import(
        '../getters/filesystem/get-content-items'
      );
  ...
  private async applyModelFiltering(
    ...
    console.log(
      `🔍 ${useFullDependencyTree ? 'Model dependency tree' : 'Simple model'} filtering: ${modelNames.join(', ')}`
    );
    ...
    if (validation.invalid.length > 0) {
      ...
      console.log(
        ansiColors.gray(
          `Available models: ${guidEntities.models.map((m: any) => m.referenceName).join(', ')}`
        )
      );
  ...
  private filterGuidEntitiesByDependencyTree(
    ...
    return {
      ...
      containers: guidEntities.containers.filter((c: any) =>
        dependencyTree.containers.has(c.contentViewID)
      ),
      ...
      assets: guidEntities.assets.filter((a: any) =>
        dependencyTree.assets.has(a.url || a.originUrl || a.edgeUrl)
      ),
      galleries: guidEntities.galleries.filter((g: any) =>
        dependencyTree.galleries.has(g.galleryID)
      ),
    };
  }
  ...
  validateDataStructure(locale: string): boolean {
    ...
    if (!fs.existsSync(instancePath)) {
      console.error(
        ansiColors.red(`❌ Data directory not found for GUID ${this.guid}: ${instancePath}`)
      );
      console.log(ansiColors.yellow(`💡 Make sure you have pulled data first:`));
      console.log(
        `   node dist/index.js pull --guid ${this.guid} --locale ${locale} --channel website --verbose`
      );
      return false;
    }
