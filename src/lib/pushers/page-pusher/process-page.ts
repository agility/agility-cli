...
export async function processPage({
  ...
  try {
    ...
    if (page.pageType !== 'folder' && page.templateName) {
      ...
      let templateRef = templateMapper.getTemplateMappingByPageTemplateName(
        page.templateName,
        'source'
      );
      if (!templateRef) {
    ...
    const websiteChannel = sitemap?.find(
      (channelObj) => channelObj.name.toLowerCase() === channel.toLowerCase()
    );
    if (websiteChannel) {
      ...
      if (Array.isArray(zoneModules)) {
        for (const module of zoneModules) {
          ...
          if (module.item && typeof module.item === 'object') {
            ...
            if (sourceContentId && sourceContentId > 0) {
              const { targetContentID } = contentMapper.getContentItemMappingByContentID(
                sourceContentId,
                'source'
              );
              if (targetContentID) {
    ...
    if (contentIdsToValidate.length > 0) {
      const mappingResults: {
        [contentId: number]: { found: boolean; targetId?: number; error?: string };
      } = {};
      let foundMappings = 0;
    ...
    if (totalModules === 0) {
      ...
      if (originalModuleCount > 0 && !existingPage && !isLegitimateEmptyPage(page)) {
        console.error(
          `✗ Page "${page.name}" lost all ${originalModuleCount} modules during content mapping`
        );
        return 'failure';
      }
    }
    ...
    const pageIDInOtherLocale = mappingToOtherLocale
      ? mappingToOtherLocale.PageIDOtherLanguage
      : -1;
    ...
    if (Array.isArray(savePageResponse) && savePageResponse.length > 0) {
      ...
      const { successfulItems: batchSuccessItems, failedItems: batchFailedItems } =
        extractBatchResults(completedBatch, [page]);
      ...
      if (batchSuccessItems.length > 0) {
        ...
        logger.page.error(
          page,
          `✗ Page ${page.name} batch failed: ${batchFailedItems[0].error}`,
          locale,
          channel,
          targetGuid
        );
      ...
      if (actualPageID > 0) {
        ...
        if (completedBatch.errorData && completedBatch.errorData.trim()) {
          logger.page.error(
            page,
            `✗ Page "${page.name}" failed  - ${completedBatch.errorData}, locale:${locale}`,
            locale,
            channel,
            targetGuid
          );
          ...
          logger.page.error(
            page,
            `✗ Page "${page.name}" failed - invalid page ID: ${actualPageID}, locale:${locale}`,
            locale,
            channel,
            targetGuid
          );
      ...
      logger.page.error(
        page,
        `✗ Page "${page.name}" failed in locale:${locale} - unexpected response format`,
        locale,
        channel,
        targetGuid
      );
    ...
    logger.page.error(
      page,
      `✗ Page "${page.name}" failed in locale:${locale} - ${error.message}`,
      locale,
      channel,
      targetGuid
    );
