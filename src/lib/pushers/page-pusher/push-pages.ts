...
export async function pushPages(sourceData: mgmtApi.PageItem[], locale: string): Promise<PusherResult> {
  ...
  if (publishableIds.length !== uniquePublishableIds.length && state.verbose) {
    console.log(
      ansiColors.gray(
        `    📋 Deduplicated publishable page IDs: ${publishableIds.length} → ${uniquePublishableIds.length}`
      )
    );
  }
