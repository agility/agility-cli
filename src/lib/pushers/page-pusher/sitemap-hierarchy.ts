...
export class SitemapHierarchy {
  ...
  loadAllSitemaps(guid: string, locale: string): { [key: string]: SitemapNode[] | null } {
    ...
    const sitemapDir = path.join(rootPath, guid, locale, 'nestedsitemap');
   ...
   */
  findPageParentInSourceSitemap(
    pageId: number,
    pageName: string,
    channelName: string
  ): { parentId: number | null; parentName: string | null; foundIn: string } {
    try {
      ...
      const searchSitemap = (
        nodes: SitemapNode[],
        parentNode: SitemapNode | null = null
      ): { parentId: number | null; parentName: string | null; foundIn: string } => {
        for (const node of nodes) {
          ...
          if (node.pageID === pageId || node.name === pageName) {
            if (parentNode) {
              console.log(
                `🎯 [DEBUG] Found ${pageName} (ID:${pageId}) under parent ${parentNode.name} (ID:${parentNode.pageID})`
              );
              return {
          ...
          if (node.children && node.children.length > 0) {
            ...
            if (dynamicMatch) {
              console.log(
                `🎯 [DEBUG] Found dynamic page ${pageName} (ID:${pageId}) under parent ${node.name} (ID:${node.pageID})`
              );
              return {
  ...
  getProcessingOrder(
    pages: any[],
    hierarchy: PageHierarchy
  ): { orderedPages: any[]; depthInfo: Map<number, number> } {
  ...
  validateProcessingOrder(orderedPages: any[], hierarchy: PageHierarchy): boolean {
    ...
    for (const page of orderedPages) {
      ...
      if (parentId && !processedPageIds.has(parentId)) {
        ...
        console.error(
          `❌ Invalid processing order: Page ${page.pageID} scheduled before parent ${parentId}`
        );
        return false;
      }
