...
describe('ContentBatchProcessor.processBatches — batch-level API failure', () => {
  it('records all items in the failing batch as failed', async () => {
    ...
    const result = await processor.processBatches(
      [makeContentItem(1), makeContentItem(2)],
      makeLogger(),
      'Test'
    );
...
describe('ContentBatchProcessor.processBatches — successful batch', () => {
  ...
  it('mixes success and failure item counts across a batch', async () => {
    ...
    mockExtract.mockReturnValue({
      ...
      failedItems: [
        { originalItem: item2, error: 'validation error' },
      ],
    });
...
describe('ContentBatchProcessor.processBatches — publishableIds', () => {
  ...
  it('logs a message about skipping staging items', async () => {
    const consoleSpy = jest.spyOn(console, 'log');
    const stagingItem = makeContentItem(1, 1); // state=1
    ...
    mockExtract.mockReturnValue({
      successfulItems: [
        { originalItem: stagingItem, newItem: { itemID: 99, processedItemVersionID: 1 }, newId: 99 },
      ],
      failedItems: [],
    });
    ...
    expect(consoleSpy).toHaveBeenCalledWith(
      expect.stringContaining('Skipping auto-publish')
    );
  });
...
describe('ContentBatchProcessor — referenceMapper.addMapping side effect', () => {
  it('calls addMapping for each successful item', async () => {
    ...
    mockExtract.mockReturnValue({
      successfulItems: [
        { originalItem: item, newItem: { itemID: 200, processedItemVersionID: 3 }, newId: 200 },
      ],
      failedItems: [],
    });
...
describe('ContentBatchProcessor.processBatches — failed item logging', () => {
  ...
  it('calls logger.content.created for each successful item', async () => {
    ...
    mockExtract.mockReturnValue({
      successfulItems: [
        { originalItem: item, newItem: { itemID: 600, processedItemVersionID: 1 }, newId: 600 },
      ],
      failedItems: [],
    });
