...
export class ContentBatchProcessor {
  ...
  private async prepareContentPayloads(
    ...
    for (const contentItem of contentBatch) {
      if (contentItem.properties.definitionName.toLowerCase() === "richtextarea" && contentItem.fields.textblob) {
        ...
        try {
          ...
          if (isTargetContentItemDeleted && state.overwrite) {
            existingContentID = existingMapping ? existingMapping?.targetContentID : -1;
          } else {
