...
export class ContentBatchProcessor {
  ...
  async processBatches(
    ...
    for (let i = 0; i < contentBatches.length; i++) {
      ...
      try {
        ...
        const { payloads: contentPayloads, skippedCount: batchSkippedCount } =
          await this.prepareContentPayloads(
            contentBatch,
        ...
        if (this.config.onBatchComplete) {
          try {
            ...
            console.warn(
              `⚠️ Batch completion callback failed for batch ${batchNumber}: ${callbackError.message}`
            );
  ...
  private async prepareContentPayloads(
    ...
    for (const contentItem of contentBatch) {
      if (
        contentItem.properties.definitionName.toLowerCase() === 'richtextarea' &&
        ...
        try {
          ...
          if (!modelMapping) {
            throw new Error(
              `Target model mapping not found for: ${sourceModel.referenceName} (ID: ${sourceModel.id})`
            );
          }
          ...
          const existingTargetContentItem = this.config.referenceMapper.getMappedEntity(
            existingMapping,
            'target'
          );

          let existingContentID = existingTargetContentItem
            ? existingTargetContentItem.contentID
            : -1;
