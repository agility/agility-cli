...
jest.mock('lib/downloaders/store-interface-filesystem', () => ({
...
describe('downloadSyncSDKByLocaleAndChannel', () => {
  ...
  it('completes without throwing given valid guid, channel, and locale', async () => {
    await expect(
      downloadSyncSDKByLocaleAndChannel('test-guid-u', 'website', 'en-us')
  ...
  it('calls getSyncClient with the expected guid', async () => {
    ...
    expect(agilitySync.getSyncClient).toHaveBeenCalledWith(
      expect.objectContaining({ guid: 'test-guid-u' })
    );
  });
  ...
  it('configures the store with an interface and options', async () => {
    ...
    const calledConfig = agilitySync.getSyncClient.mock.calls[
      agilitySync.getSyncClient.mock.calls.length - 1
    ][0];
