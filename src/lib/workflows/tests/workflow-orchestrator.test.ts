...
function makeProcessResult(overrides: Partial<processBatchesModule.BatchProcessingResult> = {}): processBatchesModule.BatchProcessingResult {
    return {
        total: 0,
        processed: 0,
        failed: 0,
...
describe('workflowOrchestrator', () => {
    ...
    describe('when items are provided and succeed', () => {
        it('returns success=true with processed counts from batch results', async () => {
            jest.spyOn(processBatchesModule, 'processBatches')
        ...
        it('returns populated logLines collected from both content and page results', async () => {
            jest.spyOn(processBatchesModule, 'processBatches')
                .mockResolvedValueOnce(makeProcessResult({ total: 1, processed: 1, processedIds: [1], logLines: ['content-log'] }))
                .mockResolvedValueOnce(makeProcessResult({ total: 1, processed: 1, processedIds: [2], logLines: ['page-log'] }));
    ...
    describe('when processContent is false', () => {
        it('skips calling processBatches for content', async () => {
            const batchSpy = jest.spyOn(processBatchesModule, 'processBatches').mockResolvedValue(
                makeProcessResult({ total: 1, processed: 1, processedIds: [10] })
            );
    ...
    describe('when processPages is false', () => {
        it('skips calling processBatches for pages', async () => {
            const batchSpy = jest.spyOn(processBatchesModule, 'processBatches').mockResolvedValue(
                makeProcessResult({ total: 1, processed: 1, processedIds: [1] })
            );
    ...
    describe('when batches partially fail', () => {
        it('returns success=false when errors accumulate', async () => {
            jest.spyOn(processBatchesModule, 'processBatches').mockImplementation(
                async (_ids, _type, _locale, _operation, errors) => {
    ...
    describe('error accumulation', () => {
        it('collects errors from both content and page batch processing', async () => {
            jest.spyOn(processBatchesModule, 'processBatches').mockImplementation(
                async (_ids, type, _locale, _operation, errors) => {
