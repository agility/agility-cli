...
describe("refreshAndUpdateMappings", () => {
  ...
  describe("when valid API keys exist for the target", () => {
    ...
    it("does not throw when pull fails", async () => {
      ...
      await expect(refreshAndUpdateMappings([1], [], "src-guid", "tgt-guid", "en-us")).resolves.not.toThrow();
    });
    ...
    it("does not throw when updateMappingsAfterPublish rejects", async () => {
      ...
      stubPullSuccess();
      jest
        .spyOn(mappingVersionUpdater, "updateMappingsAfterPublish")
        .mockRejectedValue(new Error("mapping update failed"));
