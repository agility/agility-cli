...
describe('WorkflowOperation.executeFromMappings', () => {
    ...
    describe('dry run mode', () => {
        it('returns without calling workflowOrchestrator when dryRun=true', async () => {
            ...
            jest.spyOn(mappingReader, 'readMappingsForGuidPair').mockReturnValue(
                makeMappingResult({ contentIds: [1, 2, 3], pageIds: [10, 11] })
            );
    ...
    describe('publish operation with source status check', () => {
        it('filters content to only published-in-source IDs', async () => {
            ...
            jest.spyOn(mappingReader, 'readMappingsForGuidPair').mockReturnValue(
                makeMappingResult({ contentIds: [1, 2], pageIds: [] })
            );
            ...
            const orchestratorSpy = jest.spyOn(workflowOrchestratorModule, 'workflowOrchestrator')
                .mockResolvedValue(makeOrchestratorResult({ contentResults: { total: 1, processed: 1, failed: 0, batches: 1, processedIds: [1], logLines: [] } }));
    ...
    describe('non-publish operation', () => {
        it('passes all mapped IDs to workflowOrchestrator without source status check', async () => {
            ...
            jest.spyOn(mappingReader, 'readMappingsForGuidPair').mockReturnValue(
                makeMappingResult({ contentIds: [1, 2], pageIds: [10] })
            );
