...
describe("processBatches", () => {
  ...
  describe("successful batch", () => {
    it("accumulates processedIds and increments processed count", async () => {
      jest
        .spyOn(batchWorkflowsModule, "batchWorkflow")
        .mockResolvedValue(makeBatchResult({ processedIds: [10, 20, 30] }));
    ...
    it("returns populated logLines", async () => {
      jest.spyOn(batchWorkflowsModule, "batchWorkflow").mockResolvedValue(makeBatchResult({ processedIds: [5] }));
  ...
  describe("failed batch", () => {
    it("increments failed count and pushes to errors array on batch failure", async () => {
      jest
        .spyOn(batchWorkflowsModule, "batchWorkflow")
        .mockResolvedValue(makeBatchResult({ success: false, error: "API timeout", processedIds: [] }));
  ...
  describe("batch type labeling", () => {
    it('uses "Content" label for content type', async () => {
      jest
        .spyOn(batchWorkflowsModule, "batchWorkflow")
        .mockResolvedValue(makeBatchResult({ success: false, error: "err", processedIds: [] }));
