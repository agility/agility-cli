...
export async function refreshAndUpdateMappings(
  ...
  if (!hasValidTargetKeys(targetGuid)) {
    const warnLine = ansiColors.yellow(
      `  ⚠️ No API keys available for target ${targetGuid} - skipping refresh and mapping updates`
    );
  ...
  try {
    ...
    const errorLine = ansiColors.yellow(
      `  ⚠️ Warning: Could not refresh/update mappings after publish: ${error.message}`
    );
