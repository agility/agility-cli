  ...
  try {
    ...
    for (const item of contentItems) {
      if (ids.includes(item.contentID)) {
        ...
        const displayName =
          item.fields?.title || item.fields?.name || item.properties?.referenceName || `Item ${item.contentID}`;
...
export async function processBatches(
  ...
  if (duplicatesRemoved > 0) {
    logLine(
      ansiColors.gray(
        `    📋 Deduplicated ${label.toLowerCase()} IDs: ${ids.length} → ${uniqueIds.length} (${duplicatesRemoved} duplicates removed)`
      ),
      logLines
    );
  }
  ...
  for (let i = 0; i < batches.length; i++) {
    ...
    logLine(
      ansiColors.gray(
        `[${progress}%] ${label} batch ${batchNum}/${batches.length}: ${operationName}ing ${batch.length} items...`
      ),
      logLines
    );
    ...
    try {
      ...
      if (batchResult.success) {
        ...
        if (batchResult.partialSuccess) {
          ...
          errors.push(
            `${label} batch ${batchNum} (ID: ${batchId}): Completed with errors - ${successCount} succeeded, ${failureCount} failed`
          );
        }
      } else {
  ...
  if (results.processed > results.total) {
    ...
    summaryLine = ansiColors.green(
      `✓ ${label} ${operationVerb}: ${results.processed} items (${results.total} requested + ${results.processed - results.total} nested)`
    );
