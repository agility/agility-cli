  ...
  async executeFromMappings(): Promise<WorkflowOperationResult> {
    ...
    try {
      ...
      if (useExplicitIDs) {
        ...
        console.log(
          ansiColors.gray(
            `  Explicit content IDs: ${explicitContentIds.length > 0 ? explicitContentIds.join(", ") : "(none)"}`
          )
        );
        console.log(
          ansiColors.gray(`  Explicit page IDs: ${explicitPageIds.length > 0 ? explicitPageIds.join(", ") : "(none)"}`)
        );
        ...
        if (operationType === WorkflowOperationType.Publish) {
          ...
          console.log(
            ansiColors.gray(
              `Content: ${contentPublishedInSource}/${explicitContentIds.length} published in source (${contentSkipped} staging/unpublished skipped)`
            )
          );
          console.log(
            ansiColors.gray(
              `Pages: ${pagesPublishedInSource}/${explicitPageIds.length} published in source (${pagesSkipped} staging/unpublished skipped)`
            )
          );
        ...
        if (summary.totalContent === 0 && summary.totalPages === 0) {
          ...
          console.log(
            ansiColors.gray(
              "   Run a sync operation first to create mappings, or use --contentIDs/--pageIDs to specify IDs directly."
            )
          );
          result.elapsedTime = Date.now() - startTime;
          return result;
        ...
        if (operationType === WorkflowOperationType.Publish) {
          ...
          if (publishStatus.errors.length > 0) {
            console.log(
              ansiColors.yellow(`${publishStatus.errors.length} items not found in source files (will be included)`)
            );
          }
          ...
          console.log(
            ansiColors.gray(
              `Content: ${contentPublishedInSource}/${totalContentMapped} published in source (${contentSkipped} staging/unpublished skipped)`
            )
          );
          console.log(
            ansiColors.gray(
              `Pages: ${pagesPublishedInSource}/${totalPagesMapped} published in source (${pagesSkipped} staging/unpublished skipped)`
            )
          );
      ...
      const modeDescription =
        options.processContent && options.processPages
          ? "content and pages"
      ...
      if (
        operationType === WorkflowOperationType.Publish &&
