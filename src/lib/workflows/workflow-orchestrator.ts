...
export async function workflowOrchestrator(
  ...
  if (totalRequested > 0) {
    if (totalNested > 0) {
      logLine(
        ansiColors.cyan(
          `\nWorkflow summary: ${totalProcessed} items ${operationVerb} (${totalRequested} requested + ${totalNested} nested)`
        ),
        logLines
      );
    } else {
      logLine(
        ansiColors.cyan(`\nWorkflow summary: ${totalProcessed}/${totalRequested} items ${operationVerb} successfully`),
        logLines
      );
    }
    if (totalFailed > 0) {
