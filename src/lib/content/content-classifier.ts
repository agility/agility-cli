...
export class ContentClassifier {
   ...
   */
  private hasLinkedContentReferences(
  ...
  getClassificationStats(classification: ContentClassification): string {
    ...
    const normalPercent = Math.round(
      (classificationDetails.normalCount / classificationDetails.totalItems) * 100
    );
    const linkedPercent = Math.round(
      (classificationDetails.linkedCount / classificationDetails.totalItems) * 100
    );
