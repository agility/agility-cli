...
export class ContentFieldValidator {
  ...
  private validateObjectField(
    ...
    if (fieldValue.referencename && fieldValue.sortids) {
      ...
      if (options.sourceContainers) {
        const containerExists = options.sourceContainers.some((c) => c.referenceName === fieldValue.referencename);
        if (!containerExists) {
  ...
  private validateStringField(
    ...
    if (this.isAssetUrl(fieldValue, options)) {
      if (!this.isValidAssetUrl(fieldValue)) {
        ...
        const assetExists = options.sourceAssets.some(
          (asset) => asset.originUrl === fieldValue || asset.url === fieldValue || asset.edgeUrl === fieldValue
        );
        if (!assetExists) {
    ...
    if (this.isContentIdField(fieldKey, fieldValue)) {
      const contentIds = fieldValue.includes(",") ? fieldValue.split(",").map((id) => id.trim()) : [fieldValue.trim()];
