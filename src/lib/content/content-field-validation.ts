...
export class ContentFieldValidator {
  ...
  private validateObjectField(
    ...
    if (fieldValue.referencename && fieldValue.sortids) {
      ...
      if (options.sourceContainers) {
        const containerExists = options.sourceContainers.some(
          (c) => c.referenceName === fieldValue.referencename
        );
        if (!containerExists) {
          result.warnings.push(
            `Container reference ${fieldValue.referencename} not found in field ${fieldKey}`
          );
        }
      }
    }
  ...
  private validateStringField(
    ...
    if (fieldValue.includes('cdn.aglty.io')) {
      if (!this.isValidAssetUrl(fieldValue)) {
        ...
        const assetExists = options.sourceAssets.some(
          (asset) =>
        ...
        if (!assetExists) {
          result.warnings.push(
            `Asset URL not found in source data for field ${fieldKey}: ${fieldValue}`
          );
        }
      }
    }
    ...
    if (fieldValue.length > 10000) {
      // Agility CMS typical max field length
      result.warnings.push(
        `Field ${fieldKey} exceeds recommended length (${fieldValue.length} chars)`
      );
    }
  ...
  private isContentIdField(fieldKey: string, fieldValue: string): boolean {
    ...
    return (
      (lowercaseKey.includes('categoryid') ||
