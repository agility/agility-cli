...
describe("GalleryMapper.hasSourceChanged", () => {
  ...
  it("returns true when source date is newer than mapped date", () => {
    ...
    expect(mapper.hasSourceChanged(makeGallery({ mediaGroupingID: 10, modifiedOn: "06/01/2025 10:00AM" }))).toBe(true);
  });
});
...
describe("GalleryMapper.hasTargetChanged", () => {
  ...
  it("returns false when modifiedOn has not changed", () => {
    ...
    mapper.addMapping(makeGallery({ mediaGroupingID: 10 }), makeGallery({ mediaGroupingID: 20, modifiedOn: date }));
  ...
  it("returns true when target date is newer than mapped date", () => {
    ...
    expect(mapper.hasTargetChanged(makeGallery({ mediaGroupingID: 20, modifiedOn: "12/31/2025 11:59PM" }))).toBe(true);
  });
});
