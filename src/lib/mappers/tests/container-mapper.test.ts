...
describe('ContainerMapper.hasSourceChanged', () => {
  ...
  it('returns true when source date is newer than mapped date', () => {
    ...
    expect(
      mapper.hasSourceChanged(makeContainer({ contentViewID: 10, lastModifiedDate: '06/01/2025 10:00AM' }))
    ).toBe(true);
  });
});
...
describe('ContainerMapper.hasTargetChanged', () => {
  ...
  it('returns true when target date is newer than mapped date', () => {
    ...
    expect(
      mapper.hasTargetChanged(makeContainer({ contentViewID: 20, lastModifiedDate: '12/01/2025 10:00AM' }))
    ).toBe(true);
  });
});
