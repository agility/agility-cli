...
export class PageMapper {
  ...
  addMapping(sourcePage: mgmtApi.PageItem, targetPage: mgmtApi.PageItem) {
    ...
    if (targetMapping && sourceMapping && targetMapping !== sourceMapping) {
      throw new Error(
        `Invalid Mappings detected! Source pageID: ${sourcePage.pageID}, Target pageID: ${targetPage.pageID}`
      );
    }
  ...
  updateMapping(sourcePage: mgmtApi.PageItem, targetPage: mgmtApi.PageItem, mapping: PageMapping) {
    if (targetPage.pageID !== mapping.targetPageID) {
      throw new Error(
        `Invalid items trying to be mapped! Source pageID: ${sourcePage.pageID}, Target pageID: ${targetPage.pageID}`
      );
    }
  ...
  hasTargetChanged(
    targetPage: mgmtApi.PageItem | null,
    mapping: PageMapping | null
  ): "version_changed" | "file_missing" | null {
    if (!mapping) return null;
