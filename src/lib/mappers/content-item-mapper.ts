...
export class ContentItemMapper {
  ...
  getContentItemMapping(
    ...
    type: 'source' | 'target'
  ): ContentItemMapping | null {
    const mapping = this.mappings.find((m: ContentItemMapping) =>
      type === 'source'
        ? m.sourceContentID === contentItem.contentID
        : m.targetContentID === contentItem.contentID
    );
  ...
  loadMapping() {
    const mapping = this.fileOps.getMappingFile(
      this.directory,
      this.sourceGuid,
      this.targetGuid,
      this.locale
