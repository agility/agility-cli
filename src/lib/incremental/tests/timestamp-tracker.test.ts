...
describe("loadLastPullTimestamps", () => {
  ...
  it("skips (and warns about) invalid timestamp values", () => {
    ...
    const filePath = timestampFilePath(rootPath, guid);
    fs.mkdirSync(path.dirname(filePath), { recursive: true });
    fs.writeFileSync(filePath, JSON.stringify({ models: "not-a-date", assets: "2025-03-01T00:00:00.000Z" }), "utf-8");
...
describe("isEntityModifiedSinceLastPull", () => {
  ...
  it("returns true when entity was modified after last pull", () => {
    expect(isEntityModifiedSinceLastPull("2025-06-02T00:00:00.000Z", "2025-06-01T00:00:00.000Z")).toBe(true);
  });

  it("returns false when entity was modified before last pull", () => {
    expect(isEntityModifiedSinceLastPull("2025-05-31T00:00:00.000Z", "2025-06-01T00:00:00.000Z")).toBe(false);
  });
