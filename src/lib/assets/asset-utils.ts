  ...
  try {
    ...
    try {
      ...
      if (typeof originUrl === 'string' && originUrl.startsWith('/')) {
        ...
        console.error(
          `[Asset Utils] Cannot parse originUrl: ${originUrl}. It is not a full URL and does not start with /.`
        );
    ...
    if (assetsIndex !== -1) {
      ...
      console.warn(
        `[Asset Utils] Unexpected pathname format (not starting with '/' after URL parse failed): ${pathname}. Using it directly.`
      );
