...
describe("getAssetsFromFileSystem", () => {
  ...
  it("returns combined assets from multiple JSON files", () => {
    ...
    fs.writeFileSync(path.join(jsonDir, "file2.json"), JSON.stringify({ assetMedias: [{ mediaID: 20 }] }));
  ...
  it("skips files where assetMedias is absent", () => {
    ...
    fs.writeFileSync(path.join(jsonDir, "with-medias.json"), JSON.stringify({ assetMedias: [{ mediaID: 5 }] }));
