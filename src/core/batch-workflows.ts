 ...
 */
async function withProgressIndicator<T>(promise: Promise<T>, label: string): Promise<T> {
  let dotCount = 0;
  ...
  while (retryCount < maxRetries) {
    try {
      ...
      if (batch.batchState === 3) {
        ...
        const elapsed = Math.round((Date.now() - startTime) / 1000);
        process.stdout.write(
          ansiColors.yellow.dim(
            `\r${batchType} batch ${batchID}: ${createProgressBar(100)} ${totalItems}/${totalItems} (${elapsed}s)\n`
          )
        );
      ...
      process.stdout.write(
        ansiColors.yellow.dim(
          `\r${batchType} batch ${batchID}: ${createProgressBar(percentComplete)} ${numProcessed}/${totalItems} (${elapsed}s)        `
        )
      );
...
export async function batchWorkflow(
  ...
  try {
    ...
    if (error.response) {
      ...
      if (error.response.data) {
        console.error(
          ansiColors.gray(`    Response: ${JSON.stringify(error.response.data, null, 2).substring(0, 500)}`)
        );
      }
    }
