...
export class Push {
  ...
  async pushInstances(fromSync: boolean = false): Promise<{ success: boolean; results: any[]; elapsedTime: number }> {
    ...
    try {
      ...
      const logFilePaths = results.map((res) => res.logFilePath).filter((path) => path) as string[];
   ...
   */
  private async executeAutoPublish(
    results: PushResults[],
    autoPublishMode: string
  ): Promise<Array<{ locale: string; type: string; error: string }>> {
    ...
    let totalContent = 0;
    let totalPages = 0;
    contentIdsByLocale.forEach((ids) => {
      if (publishContent) totalContent += new Set(ids).size;
    });
    pageIdsByLocale.forEach((ids) => {
      if (publishPages) totalPages += new Set(ids).size;
    });
    ...
    try {
      ...
      for (const locale of Array.from(allLocales)) {
        ...
        if (contentIds.length > 0) {
          const contentResult = await processBatches(
            contentIds,
            "content",
            locale,
