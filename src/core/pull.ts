...
import { overwritePrompt } from '../lib/prompts'; // Import the new prompt
import { getState } from './state';
const storeInterfaceFileSystem = require("../lib/downloaders/store-interface-filesystem"); 
...
export class Pull {
  ...
  async pullInstance(): Promise<void> {
    ...
    const totalStartTime = Date.now(); // Track total runtime for completion message
    ...
    // Enhanced progress tracking setup moved to downloadAllSyncSDK
    ...
    for (let i = 0; i < pullSteps.length; i++) {
        ...
        // Simplified start of step logging - skip confusing messages for Content Sync SDK handled steps
        const isContentSyncStep = ['Pages', 'Sitemaps', 'Redirections'].includes(stepName);
        if (!isContentSyncStep) {
            if (this.isVerbose) {
        ...
        try {
            ...
            if (stepName === 'Content') {
                await downloadAllSyncSDK(this._multibar!, this.fileOps, this._update, stepProgressCallback);
                continue; 
            }
    ...
    if (this._useBlessedUI && screen) {
        ...
        const summaryMessage = `Pull completed: ${finalSuccessCount}/${finalTotalSteps} steps successful, ${finalErrorCount} errors, ${timeDisplay}`;
        ...
        const summaryMessage = `Pull operation completed: ${finalSuccessCount}/${finalTotalSteps} steps successful, ${finalErrorCount} errors, ${timeDisplay}`;
        console.log(ansiColors.green(summaryMessage));
